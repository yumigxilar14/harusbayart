...
async function loginWithCookie() {
    try {
        ...
        const browser = await puppeteer.launch({ headless: false, executablePath: 'C:\\Users\\User\\.cache\\puppeteer\\chrome\\win64-124.0.6367.201\\chrome-win64\\chrome.exe' }); // Sesuaikan path executable Chrome

        // Lakukan login pada setiap tab baru
        for (let i = 0; i < 50; i++) {
            const page = await browser.newPage();
            await page.setCookie(...cookies);
            await page.goto('https://id.shp.ee/93gm8Ax');
